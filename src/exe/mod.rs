...
use prometheus::{Encoder, Gauge, register_gauge, TextEncoder};
...
lazy_static! {
    static ref TOTAL_CHANNELS: Gauge = register_gauge!(
        "total_channels",
...
pub async fn run<
    ...
    tokio::spawn( async move {
        ...
        let service = make_service_fn(|_| async { Ok::<_, hyper::Error>(service_fn(private_routes)) });
